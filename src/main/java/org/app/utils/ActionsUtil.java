...
public class ActionsUtil {
    ...
    public static void scrollToElement(AndroidDriver driver, By locator) {
        WaitUtil.waitForElementPresence(driver, locator);
        try {
            driver.findElement(AppiumBy.androidUIAutomator(
                    "new UiScrollable(new UiSelector().scrollable(true).instance(0))" +
                            ".scrollIntoView(new UiSelector().text(\"" + locator + "\"))"));
            LogsUtil.info("Scrolled to element", locator.toString());
        } catch (Exception e) {
            LogsUtil.error("Failed to scroll to element", locator.toString());
        }
    }
